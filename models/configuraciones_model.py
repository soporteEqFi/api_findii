...
class ConfiguracionesModel:
    ...
    def obtener_categorias_disponibles(self, *, empresa_id: int) -> List[str]:
        """Obtener lista de categorías disponibles para una empresa"""
        try:
            resp = supabase.table(self.TABLE).select("categoria").eq("empresa_id", empresa_id).eq("activo", True).execute()
            data = _get_data(resp)
            ...
            print(f"❌ Error obteniendo categorías disponibles: {e}")
            return []
    ...
    def actualizar_categoria(self, *, empresa_id: int, categoria: str, configuracion: Any, descripcion: str) -> Dict[str, Any]:
        """Actualizar configuración de una categoría existente o crear nueva"""
        try:
            # Buscar la configuración existente
            resp = supabase.table(self.TABLE).select("id").eq("empresa_id", empresa_id).eq("categoria", categoria).execute()
            data = _get_data(resp)
            ...
            print(f"❌ Error actualizando configuración {categoria}: {e}")
            return {}

    def crear_categoria(self, *, empresa_id: int, categoria: str, configuracion: Any, descripcion: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data and len(data) > 0:
                raise ValueError(f"La categoría '{categoria}' ya existe")
            ...
            print(f"❌ Error creando configuración {categoria}: {e}")
            raise e

    def eliminar_categoria(self, *, empresa_id: int, categoria: str) -> Dict[str, Any]:
        """Eliminar categoría de configuración (marcar como inactiva)"""
        try:
            resp = supabase.table(self.TABLE).update({
                "activo": False,
                "updated_at": "now()"
            }).eq("empresa_id", empresa_id).eq("categoria", categoria).execute()

            updated_data = _get_data(resp)
            return updated_data[0] if updated_data else {}

        except Exception as e:
            print(f"❌ Error eliminando configuración {categoria}: {e}")
            return {}